...
### AI Mode (Gemini Node Labels) Is Not Yet Available

The **AI (Gemini)** node label mode — which generates 8-word summaries for nodes using the Gemini API — is **not functional in this version**. Setting Node Labels to "AI (Gemini)" in Settings will have no effect; nodes will continue to use local labels (first 60 characters of your prompt).

This feature is under active development and will be enabled in a future update. There is no action needed on your part — when the feature is ready, it will work with any Gemini API key you have already saved in Settings.
...
**AI (Gemini)** *(coming soon — not functional in this version)*: Will generate 8-word summaries using Gemini 2.0 Flash. Requires an API key. Currently selecting this mode has no effect; nodes fall back to local labels regardless. This will be enabled in a future update.

### Gemini API Key

Paste your Gemini API key (starts with `AIza...`) here. The key is stored in IndexedDB and **never leaves your browser**.

> **AI mode is not yet functional in this version.** You can save your API key now and it will be ready when AI mode is enabled in a future update. No API calls are currently made regardless of the key or mode selected.
