...
5. [The Popup](#the-popup)
6. [The Sidebar](#the-sidebar)
7. [How the Tree Is Built](#how-the-tree-is-built)
8. [Understanding Drift Detection](#understanding-drift-detection)
9. [Node Types and Colors](#node-types-and-colors)
10. [Navigating the Canvas](#navigating-the-canvas)
11. [The Node Detail Panel](#the-node-detail-panel)
12. [The "✂ Branch Here" Button](#the--branch-here-button)
13. [Settings](#settings)
14. [Undo](#undo)
15. [New Tree vs. Reload](#new-tree-vs-reload)
...
### Gemini: Only Visible Messages Are Processed

Gemini loads conversation history **lazily** — older messages are not present in the DOM until you scroll up to them. Branch Barber can only process turns that are currently rendered on the page.

**What this means in practice:**

- If you open a long Gemini conversation, only the messages visible in the current viewport (and those just below it) will be in the tree. Older messages higher up the page are not loaded yet.
- To include older messages in the tree: **scroll up** in the Gemini chat until the messages you want are loaded, then click **New Tree** in the popup to rebuild. Branch Barber will process all DOM-rendered turns at that point.
- This is a Gemini platform constraint, not a Branch Barber bug. ChatGPT does not have this limitation.

### AI Mode: Rate Limits on the Gemini Free Tier

The **AI (Gemini)** node label mode works, but Gemini's free tier enforces a limit of **15 requests per minute**. Branch Barber spaces Gemini calls 4.5 seconds apart (≈ 13 RPM) to stay comfortably under this limit.

**What this means in practice:**
- Node labels update automatically ~4–5 seconds after the AI finishes responding — you will see the fallback label first (first 60 chars of your prompt), then it silently updates to the Gemini summary.
- If you send several messages very quickly (faster than one every 4 seconds), calls queue up and labels arrive with a short delay. This is expected and not an error.
- If you hit a 429 anyway, Branch Barber retries automatically with a 10s → 20s backoff before giving up and keeping the fallback label.

A paid-tier Gemini key removes the RPM constraint entirely.
...
**AI (Gemini)**: Node labels are 8-word summaries generated by Gemini 2.0 Flash. More descriptive. Requires an API key. Gemini is only called for **new turns you type after the tree opens** — historical turns always use local labels to avoid rate limiting. Calls are spaced 4.5 seconds apart to stay within the free-tier 15 RPM limit.
...
With a key configured and AI mode selected, each new turn you type gets an 8-word Gemini summary as its node label. When a branch is auto-detected or manually created, the ghost placeholder also gets a Gemini-generated label describing the parent node's topic.

To get a free key: visit [aistudio.google.com/apikey](https://aistudio.google.com/apikey).
...
- **On Gemini specifically:** older messages are only loaded into the DOM when you scroll up to them. If messages from earlier in the conversation are missing, scroll up in the chat to load them, then click "New Tree" to rebuild. See [Known Limitations](#known-limitations).
